...
```jsonc
{
  ...
  "settings": { "scenarios": {…}, "chaos": {…} },        // the portable mock settings subset
...
1. **Allowlist.** Only `scenarios` and `chaos` travel from `versions.mock_settings`. Hosted-plane
   access control (the private-mock `mode`) is meaningless offline and never leaves the server.
