...
| Cookie | `__Secure-next-auth.session-token` (prod), `httpOnly`, `sameSite=lax`, `secure`, `domain=.apiome.dev` for cross-subdomain sharing with the studio | `apiome-ui/lib/auth/cookie-options.ts:100-140` |
...
| **Cross-subdomain (`*.apiome.dev`)** | Preserved: the opaque token rides the same shared-domain cookie and is validated against the **shared** DB. Arguably cleaner than re-verifying a JWT signature in every app. | Preserved, but each consumer must hold the secret to verify. |
