-- VK provider vocabulary widen (OLO-9.42, #5055)
--
-- OLO-9.42 adds the VK ID provider (Russia / CIS country MVP). The V202 CHECKs pin a
-- vocabulary that includes `line` but not `vk`. OLO-9.16 (#5029) will widen the full
-- Better Auth catalog; until then this migration admits `vk` alone so the registry entry and
...
COMMENT ON COLUMN apiome.auth_provider_config.provider_id IS
    'Provider slug matching PROVIDER_REGISTRY ids; primary key, one row per provider. Vocabulary widened for OLO-9.42 (#5055) to accept VK (vk) ahead of the full-catalog OLO-9.16 widen.';
