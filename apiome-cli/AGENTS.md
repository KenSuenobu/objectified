...
| Contract assurance | `POST /v1/tenants/{tenant_slug}/contracts/{version_ref}/suite` (ECA-1.1); `version_ref` is the schema-reference grammar without a type segment (`project/{slug}/{version}`, `catalog/{item}/{version}`). Returns a deterministic suite manifest with a `sha256:` digest over its own canonical bytes; a version that yields no suite is a 200 with `ok: false` |
