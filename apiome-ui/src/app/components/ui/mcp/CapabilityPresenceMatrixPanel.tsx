...
function MatrixCell({
  ...
  if (state === 'added') {
    return (
      <span
        className="inline-flex size-5 items-center justify-center rounded-sm bg-ok-soft text-ok-fg"
        role="img"
        aria-label={label}
        title={label}
      >
  ...
  if (state === 'modified') {
    return (
      <span
        className="inline-flex size-5 items-center justify-center rounded-sm bg-accent-soft text-accent-fg"
        role="img"
        aria-label={label}
        title={label}
      >
  ...
  if (state === 'present') {
    return (
      <span
        className="inline-flex size-5 items-center justify-center rounded-sm bg-[color-mix(in_srgb,var(--ok)_10%,transparent)]"
        role="img"
        aria-label={label}
        title={label}
      >
  ...
  return (
    <span
      className="inline-flex h-5 w-5 items-center justify-center text-fg-faint"
      role="img"
      aria-label={label}
      title={label}
    >
  ...
  return (
    <th scope="col" className="px-1 py-2 text-center align-bottom">
      <button
        ...
        aria-label={intent}
        title={intent}
        className={`inline-flex min-w-[2.25rem] flex-col items-center rounded px-1.5 py-1 text-xs font-medium tabular-nums transition-colors hover:bg-subtle focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
          isCurrent
            ? 'text-accent-fg'
            : 'text-fg-muted'
        }`}
      >
        ...
        {isCurrent ? (
          <span className="mt-0.5 text-2xs font-normal uppercase tracking-wider text-fg-muted">
            current
          </span>
        ) : null}
      </button>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <thead>
            <tr className="border-b border-border bg-inset">
              <th
                scope="col"
                className="sticky left-0 z-10 bg-inset px-3 py-2 text-left text-xs font-medium uppercase tracking-wider text-fg-muted"
              >
                Capability
              </th>
...
function MatrixRow({ row, columns }: { row: McpMatrixRow; columns: readonly McpMatrixColumn[] }) {
  return (
    <tr className="border-b border-border last:border-0">
      <th
        scope="row"
        className="sticky left-0 z-10 max-w-[18rem] bg-surface px-3 py-1.5 text-left"
      >
        <span className="flex items-center gap-2">
          <span className="min-w-0">
            <span className="block truncate font-mono text-xs font-medium text-fg" title={row.name}>
              {row.name}
            </span>
