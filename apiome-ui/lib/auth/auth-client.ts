...
/**
 * Better Auth browser client (OLO-10.2, extended for 2FA in OLO-10.10 / OLO-9.13 #5014 / OLO-9.50 #5070).
 *
 * The React client the UI calls as `authClient.signIn` / `authClient.signOut` /
 * `authClient.useSession` (and `authClient.twoFactor.*` for TOTP / email OTP).
 *
 ...
 * `twoFactorRedirect` to `/login/2fa`, preserving the intended post-login callback and the
 * `twoFactorMethods` list (OLO-9.50) so the second step can offer Authenticator and/or email OTP.
 * OAuth/SSO second-factor is out of scope — Better Auth's stock after-hook only matches credential paths.
 *
  ...
  plugins: [
    twoFactorClient({
      onTwoFactorRedirect({ twoFactorMethods } = {}) {
        ...
        storeTwoFactorMethods(twoFactorMethods);
