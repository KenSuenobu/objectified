/**
 * Integration tests for the cross-repo discovered-spec catalog page (REPO-6.4, #2797).
 *
 * These drive the real component against a stubbed `/api/repositories/catalog` and assert the
 * behaviours the ticket's acceptance criteria name:
 *
...
describe('rendering the catalog', () => {
  test('lists every spec the server returned, across repositories', async () => {
    render(<RepositorySpecCatalog />);

    await waitFor(() => expect(screen.getAllByTestId('spec-catalog-row')).toHaveLength(2));
  ...
  test('a row links into the file drawer on its owning repository', async () => {
    ...
    const row = (await screen.findAllByTestId('spec-catalog-row'))[0];
  ...
  test('the derived status is shown per row', async () => {
    ...
    const rows = await screen.findAllByTestId('spec-catalog-row');
    expect(within(rows[0]).getByTestId('spec-catalog-status')).toHaveTextContent('Imported');
    expect(within(rows[1]).getByTestId('spec-catalog-status')).toHaveTextContent(
      'Needs attention'
    );
  });
  ...
  test('an unresolved external $ref is called out on the row that has one', async () => {
    ...
    const rows = await screen.findAllByTestId('spec-catalog-row');
    expect(within(rows[1]).getByText(/2 unresolved external \$refs/)).toBeInTheDocument();
    expect(within(rows[0]).queryByText(/unresolved external/)).not.toBeInTheDocument();
  });
  ...
  test('an unmapped spec reads as unmapped rather than blank', async () => {
    ...
    const rows = await screen.findAllByTestId('spec-catalog-row');
    expect(within(rows[1]).getByText('Unmapped')).toBeInTheDocument();
    expect(within(rows[0]).getByText('Orders API')).toBeInTheDocument();
  });
...
describe('server-side search and filtering', () => {
  ...
  test('typing a search sends it to the server rather than filtering locally', async () => {
    const user = userEvent.setup();
    render(<RepositorySpecCatalog />);
    await screen.findAllByTestId('spec-catalog-row');
    ...
    expect(screen.getAllByTestId('spec-catalog-row')).toHaveLength(2);
  });
  ...
  test('a slow earlier response cannot overwrite a faster later one', async () => {
    ...
    render(<RepositorySpecCatalog />);
    await user.type(screen.getByLabelText('Search discovered specs'), 'checkout');
    await waitFor(() => expect(screen.getAllByTestId('spec-catalog-row')).toHaveLength(1));

    release!();

    // Give the stale response a chance to land; the filtered result must survive it.
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2));
    expect(screen.getAllByTestId('spec-catalog-row')).toHaveLength(1);
  });
